...
class Statistics {
	...
	func addHookHeaders() {
		...
		if hooks["combinationAnalysis"] != nil {
			writeToHook("combinationAnalysis", data: "parkedID\(separator)combinationBinary\(separator)activeRSUs\(separator)meanSig\(separator)stdevSig\(separator)meanSat\(separator)stdevSat\(terminator)")
		}
	}
